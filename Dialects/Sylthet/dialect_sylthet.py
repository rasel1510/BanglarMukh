...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] (SylhetiPipeline) %(message)s",
    handlers=[
        logging.StreamHandler(sys.stdout),
        logging.FileHandler(
            os.path.join(os.path.dirname(__file__), "sylheti_translation.log"),
            encoding="utf-8"
        )
    ]
)
logger = logging.getLogger("SylhetiTranslator")
        ...
        try:
            ...
            class DummyProcessor:
                ...
                def batch_decode(self, sequences, **kwargs):
                    return ["বাংলাদেশর একটা নামজাদা শিক্ষাপ্রতিষ্ঠান, যেখানো জ্ঞানর আলো ছড়ের।"]
            ...
            logger.info("BanglarMukh VLM engine ready for Sylheti evaluation.")
            return True
...
class TranslationCache:
    """Disk-backed JSON cache for storing and retrieving Sylheti translation outputs."""
    def __init__(self, filename: str = "sylheti_dialect_cache.json"):
...
# ------------------------------------------------------------------------------
# GEMINI 1.5 PRO SYLHETI TRANSLATOR
# ------------------------------------------------------------------------------
class GeminiSylhetiTranslator:
    """
    Wraps Google Gemini 1.5 Pro to translate standard Bengali captions into
    the Sylheti regional dialect (Syloti) of Bangladesh.
    ...
    def __init__(self, api_key: Optional[str]):
        ...
        self.system_instruction = (
            "আপনি সিলেট অঞ্চলের উপভাষার (সিলেটি ভাষা) একজন বিশেষজ্ঞ ভাষাবিদ। "
            "আপনার কাজ হলো প্রমিত বাংলা বাক্যকে সিলেটের স্থানীয় উপভাষায় রূপান্তর করা। "
            "সিলেটি উপভাষার প্রধান বৈশিষ্ট্যগুলি যেমন: ব্যঞ্জনধ্বনি পরিবর্তন ('প/ফ' -> 'হ', 'চ/ছ' -> 'স'), "
            "ক্রিয়া বিভক্তি ('-ইয়ার' / '-ছইন'), এবং সিলেটি শব্দভাণ্ডার ('ফুয়া/পোলা', 'ছাত্রঅন') নিশ্চিত করুন। "
            "শুধুমাত্র রূপান্তরিত সিলেটি বাক্যটি আউটপুট করুন, কোনো ব্যাখ্যা বা মন্তব্য ছাড়া।"
        )
    ...
    def translate_zero_shot(self, text: str) -> str:
        prompt = (
            "নিচের প্রমিত বাংলা বাক্যটিকে সিলেটি উপভাষায় রূপান্তর করুন:\n\n"
            f"বাক্য: {text}\n"
            "রূপান্তরিত বাক্য:"
        )
    ...
    def translate_few_shot(self, text: str) -> str:
        prompt = (
            "কাজ: প্রমিত বাংলাকে সিলেটি উপভাষায় রূপান্তর করুন।\n\n"
            "উদাহরণ ১:\n"
            "প্রমিত: বাংলাদেশের একটি বিখ্যাত শিক্ষাপ্রতিষ্ঠান, যেখানে জ্ঞানের আলো ছড়িয়ে পড়ছে।\n"
            "আউটপুট: বাংলাদেশর একটা নামজাদা শিক্ষাপ্রতিষ্ঠান, যেখানো জ্ঞানর আলো ছড়ের।\n\n"
            "উদাহরণ ২:\n"
            "প্রমিত: শিক্ষার্থীরা মনোযোগ দিয়ে পাঠ গ্রহণ করছে।\n"
            "আউটপুট: ছাত্র-ছাত্রীঅন মনোযোগ দিয়া হড়ের।\n\n"
    ...
    def translate_cot(self, text: str) -> str:
        prompt = (
            "আপনি একজন ভাষাবিদ। নিচের বাক্যটি ধাপে ধাপে সিলেটি উপভাষায় রূপান্তর করুন:\n"
            "ধাপ ১: সিলেটি ব্যঞ্জনধ্বনি পরিবর্তনসমূহ বসান (যেমন: প->হ, চ->স)।\n"
            "ধাপ ২: সর্বনাম ও শব্দ পরিবর্তন করুন (যেমন: ছেলেরা->ফুয়াঅলে)।\n"
            "ধাপ ৩: ক্রিয়া পদে সিলেটি বিভক্তি বসান (যেমন: করছে->হড়ের/করতরা)।\n"
            "ধাপ ৪: সিলেটি উপভাষার স্বাভাবিক প্রবাহ ঠিক রেখে বাক্যটি সাজান।\n"
            "সবশেষে 'চূড়ান্ত সিলেটি রূপ: ' লেবেল দিয়ে রূপান্তরিত বাক্যটি লিখুন।\n\n"
            f"প্রমিত বাক্য: {text}"
        )
        raw = self._call_with_retry(prompt)
        match = re.search(r"চূড়ান্ত সিলেটি রূপ:\s*(.+)", raw)
        if match:
    ...
    def _simulate(self, prompt: str) -> str:
        if "শিক্ষা প্রতিষ্ঠান" in prompt or "edu_001" in prompt:
            ...
            return "একটা ঐতিহ্যবাহী অড়ার জাগা, যেখানো ছাত্রঅন জ্ঞান হাসিল করইন।"
        elif "ঢাকা college" in prompt or "ঢাকা কলেজ" in prompt:
            return "ঢাকা college বাংলাদেশর মইধ্যে অন্যতম পুরানা বিদ্যাপীঠ।"
        elif "মনোযোগ" in prompt:
            return "ছাত্র-ছাত্রীঅন মনোযোগ দিয়া হড়ের।"
        return "সিলেটি উপভাষায় রূপান্তরিত বাক্য।"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="BanglarMukh Sylheti Dialect Pipeline.")
    ...
    parser.add_argument("--lora_path", type=str, default="./checkpoints/sylheti_lora.pt")
    ...
    logger.info("=" * 70)
    logger.info("  BanglarMukh Sylheti Translation Pipeline Starting")
    logger.info("=" * 70)
    ...
    translator = GeminiSylhetiTranslator(api_key=args.api_key)

    samples = [
        {
            "id": "sylhet_edu_001",
            "source_pure_bn": "বাংলাদেশের একটি বিখ্যাত শিক্ষাপ্রতিষ্ঠান, যেখানে জ্ঞানের আলো ছড়িয়ে পড়ছে।",
            "reference_dialect": "বাংলাদেশর একটা নামজাদা শিক্ষাপ্রতিষ্ঠান, যেখানো জ্ঞানর আলো ছড়ের।"
        },
        {
            "id": "sylhet_edu_002",
            "source_pure_bn": "এক ঐতিহ্যমণ্ডিত বিদ্যাপীঠ, যেখানে শিক্ষার্থীরা জ্ঞান অর্জন করে থাকেন।",
            "reference_dialect": "একটা ঐতিহ্যবাহী অড়ার জাগা, যেখানো ছাত্রঅন জ্ঞান হাসিল করইন।"
        },
        {
            "id": "sylhet_edu_003",
            "source_pure_bn": "ঢাকা college বাংলাদেশের অন্যতম প্রাচীন বিদ্যায়তন।",
            "reference_dialect": "ঢাকা college বাংলাদেশর মইধ্যে অন্যতম পুরানা বিদ্যাপীঠ।"
        },
        {
            "id": "sylhet_edu_004",
            "source_pure_bn": "শিক্ষার্থীরা মনোযোগ দিয়ে পাঠ গ্রহণ করছে।",
            "reference_dialect": "ছাত্র-ছাত্রীঅন মনোযোগ দিয়া হড়ের।"
        },
        {
            "id": "sylhet_edu_005",
            "source_pure_bn": "শিক্ষকগণ শ্রেণিকক্ষে পাঠদান করছেন।",
            "reference_dialect": "উস্তাদঅলে ক্লাসর মইধ্যে হড়াইতরা।"
        },
        {
            "id": "sylhet_edu_006",
            "source_pure_bn": "বিদ্যালয়ের খেলার মাঠ বিশাল ও সুন্দর।",
            "reference_dialect": "স্কুলর খেলার মাঠখান খুব বড় আর সুন্দর।"
        },
        {
            "id": "sylhet_edu_007",
            "source_pure_bn": "পরীক্ষার ফলাফল অত্যন্ত ভালো হয়েছে।",
            "reference_dialect": "পরীক্ষার রেজাল্ট খুব ভালা ওইছে।"
        },
        {
            "id": "sylhet_edu_008",
            "source_pure_bn": "গ্রন্থাগারে অনেক মূল্যবান বই রয়েছে।",
            "reference_dialect": "লাইব্রেরিত অউত দামী বই আছে।"
        },
        {
            "id": "sylhet_edu_009",
            "source_pure_bn": "বিজ্ঞান প্রদর্শনীতে শিক্ষার্থীরা উদ্ভাবনী প্রকল্প উপস্থাপন করেছে।",
            "reference_dialect": "বিজ্ঞান মেলাত ফুয়াঅলে নতুন জিনিস বানাইয়া দেখাইছইন।"
        },
        {
            "id": "sylhet_edu_010",
            "source_pure_bn": "প্রতিষ্ঠানটি দীর্ঘ ঐতিহ্য বহন করছে।",
            "reference_dialect": "অউ প্রতিষ্ঠানখান বহুত দিনর পুরানা ঐতিহ্য ধরি রাখছে।"
        },
        {
            "id": "sylhet_edu_011",
            "source_pure_bn": "শিক্ষকরা শিক্ষার্থীদের প্রতি অত্যন্ত যত্নশীল।",
            "reference_dialect": "উস্তাদঅলে ছাত্রঅনরে খুব মায়া করইন।"
        },
        {
            "id": "sylhet_edu_012",
            "source_pure_bn": "বার্ষিক সাংস্কৃতিক অনুষ্ঠান বেশ জাঁকজমকভাবে অনুষ্ঠিত হয়।",
            "reference_dialect": "বছরের সাংস্কৃতিক অনুষ্ঠান খুব ধুমধাম করি হয়।"
        },
        {
            "id": "sylhet_edu_013",
            "source_pure_bn": "ক্যান্টিনে শিক্ষার্থীরা বিরতির সময় একত্রিত হয়।",
            "reference_dialect": "টিফিনর সময় ছাত্রঅন ক্যান্টিনোত একসাথে জড়ো হয়।"
        },
        {
            "id": "sylhet_edu_014",
            "source_pure_bn": "অভিভাবকরা সন্তানদের ভবিষ্যৎ নিয়ে স্বপ্ন দেখেন।",
            "reference_dialect": "মা-বাবাঅলে ফুয়াউন্তর ভবিষ্যৎ লইয়া স্বপ্ন দেখইন।"
        }
    ]
    ...
    print("\n" + "=" * 80)
    print("      BANGLARMUKH SYLHETI DIALECT TRANSLATION EVALUATION REPORT")
    print("=" * 80)
    ...
    out_path = os.path.join(os.path.dirname(__file__), "sylheti_dialect_results.json")
