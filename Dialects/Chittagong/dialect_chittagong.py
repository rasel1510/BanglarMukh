...
================================================================================
Dialect Target : চাটগাঁইয়া / চট্টগ্রামি (Chittagong regional dialect)
Phonetics      : Aspirate reduction, unique vowel shifts (a->o), Arakanese influence
...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] (ChittagongPipeline) %(message)s",
    handlers=[
        logging.StreamHandler(sys.stdout),
        logging.FileHandler(
            os.path.join(os.path.dirname(__file__), "chittagong_translation.log"),
            encoding="utf-8"
        )
    ]
)
logger = logging.getLogger("ChittagongTranslator")
        ...
        try:
            ...
            class DummyProcessor:
                ...
                def batch_decode(self, sequences, **kwargs):
                    return ["বাংলাদেশর একখান নামকরা স্কুল-কলেজ, হেইখানে জ্ঞানর আলো ছড়াইয়ে পড়ে।"]
            ...
            logger.info("BanglarMukh VLM engine ready for Chittagong evaluation.")
            return True
...
class TranslationCache:
    """Disk-backed JSON cache for storing and retrieving Chittagong translation outputs."""
    def __init__(self, filename: str = "chittagong_dialect_cache.json"):
...
# ------------------------------------------------------------------------------
# GEMINI 1.5 PRO CHITTAGONG TRANSLATOR
# ------------------------------------------------------------------------------
class GeminiChittagongTranslator:
    """
    Wraps Google Gemini 1.5 Pro to translate standard Bengali captions into
    the Chittagong regional dialect (Chatgaiya) of Bangladesh.
    ...
    def __init__(self, api_key: Optional[str]):
        ...
        self.system_instruction = (
            "আপনি চট্টগ্রামের চাটগাঁইয়া উপভাষার একজন বিশেষজ্ঞ ভাষাবিদ। "
            "আপনার কাজ প্রমাণ বাংলাকে চাটগাঁইয়া উপভাষায় রূপান্তর করা। "
            "চাটগাঁইয়া উপভাষার প্রধান বৈশিষ্ট্যগুলি যেমন: স্বরবর্ণ পরিবর্তন (আ→ও/অ), "
            "ক্রিয়া পদ পরিবর্তন ('-আইত্তাম', '-গেলাম', 'আছে'→'আছি/আছে'), "
            "বিশেষ সর্বনাম ('তুঁই', 'হে', 'তারা'→'হেরা') যথাযথভাবে প্রয়োগ করুন। "
            "শুধুমাত্র রূপান্তরিত চাটগাঁইয়া বাক্যটি আউটপুট করুন, কোনো ব্যাখ্যা বা মন্তব্য ছাড়া।"
        )
    ...
    def translate_zero_shot(self, text: str) -> str:
        prompt = (
            "নিচের প্রমিত বাংলা বাক্যটিকে চাটগাঁইয়া উপভাষায় রূপান্তর করুন:\n\n"
            f"বাক্য: {text}\n"
            "রূপান্তরিত বাক্য:"
        )
    ...
    def translate_few_shot(self, text: str) -> str:
        prompt = (
            "কাজ: প্রমিত বাংলাকে চাটগাঁইয়া উপভাষায় রূপান্তর করুন।\n\n"
            "উদাহরণ ১:\n"
            "প্রমিত: বাংলাদেশের একটি বিখ্যাত শিক্ষাপ্রতিষ্ঠান, যেখানে জ্ঞানের আলো ছড়িয়ে পড়ছে।\n"
            "আউটপুট: বাংলাদেশর একখান নামকরা স্কুল-কলেজ, হেইখানে জ্ঞানর আলো ছাড়াইয়ে পড়ে।\n\n"
            "উদাহরণ ২:\n"
            "প্রমিত: শিক্ষার্থীরা মনোযোগ দিয়ে পাঠ গ্রহণ করছে।\n"
            "আউটপুট: পোয়াইন মনোযোগ দিয়ে পড়া করে।\n\n"
    ...
    def translate_cot(self, text: str) -> str:
        prompt = (
            "আপনি একজন ভাষাবিদ। নিচের বাক্যটি ধাপে ধাপে চাটগাঁইয়া উপভাষায় রূপান্তর করুন:\n"
            "ধাপ ১: স্বরবর্ণের পরিবর্তন চিহ্নিত করুন (যেমন: আ→ও/অ)।\n"
            "ধাপ ২: বিশেষ্য ও সর্বনাম পরিবর্তন করুন (যেমন: তারা→হেরা)।\n"
            "ধাপ ৩: চাটগাঁইয়া উপভাষার ক্রিয়া পদ প্রয়োগ করুন।\n"
            "ধাপ ৪: চাটগাঁইয়া উপভাষার প্রবাহ ঠিক রেখে বাক্যটি গঠন করুন।\n"
            "সবশেষে 'চূড়ান্ত চাটগাঁইয়া রূপ: ' লেবেল দিয়ে রূপান্তরিত বাক্যটি লিখুন।\n\n"
            f"প্রমিত বাক্য: {text}"
        )
        raw = self._call_with_retry(prompt)
        match = re.search(r"চূড়ান্ত চাটগাঁইয়া রূপ:\s*(.+)", raw)
        if match:
    ...
    def _simulate(self, prompt: str) -> str:
        if "শিক্ষা প্রতিষ্ঠান" in prompt or "edu_001" in prompt:
            return "বাংলাদেশর একখান নামকরা স্কুল-কলেজ, হেইখানে জ্ঞানর আলো ছাড়াইয়ে পড়ে।"
        elif "বিদ্যাপীঠ" in prompt:
            return "একখান ইতিহাসর বিদ্যাপীঠ, পোয়াইন হেইখানে পড়ে।"
        elif "চট্টগ্রাম কলেজ" in prompt:
            return "চাটগাঁর কলেজ বন্দর শহরর একখান পুরানো বিদ্যাপীঠ।"
        elif "মনোযোগ" in prompt:
            return "পোয়াইন মনোযোগ দিয়ে পড়া করে।"
        return "চাটগাঁইয়া উপভাষায় রূপান্তরিত বাক্য।"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="BanglarMukh Chittagong Dialect Pipeline.")
    ...
    parser.add_argument("--lora_path", type=str, default="./checkpoints/chittagong_lora.pt")
    ...
    logger.info("=" * 70)
    logger.info("  BanglarMukh Chittagong Translation Pipeline Starting")
    logger.info("=" * 70)
    ...
    translator = GeminiChittagongTranslator(api_key=args.api_key)

    samples = [
        {
            "id": "chittagong_edu_001",
            "source_pure_bn": "বাংলাদেশের একটি বিখ্যাত শিক্ষাপ্রতিষ্ঠান, যেখানে জ্ঞানের আলো ছড়িয়ে পড়ছে।",
            "reference_dialect": "বাংলাদেশর একখান নামকরা স্কুল-কলেজ, হেইখানে জ্ঞানর আলো ছাড়াইয়ে পড়ে।"
        },
        {
            "id": "chittagong_edu_002",
            "source_pure_bn": "এক ঐতিহ্যমণ্ডিত বিদ্যাপীঠ, যেখানে শিক্ষার্থীরা পড়াশোনা করে।",
            "reference_dialect": "একখান ইতিহাসর বিদ্যাপীঠ, পোয়াইন হেইখানে পড়ে।"
        },
        {
            "id": "chittagong_edu_003",
            "source_pure_bn": "চট্টগ্রাম college বন্দরনগরীর একটি প্রাচীন বিদ্যায়তন।",
            "reference_dialect": "চাটগাঁর college বন্দর শহরর একখান পুরানো বিদ্যাপীঠ।"
        },
        {
            "id": "chittagong_edu_004",
            "source_pure_bn": "শিক্ষার্থীরা মনোযোগ দিয়ে পাঠ গ্রহণ করছে।",
            "reference_dialect": "পোয়াইন মনোযোগ দিয়ে পড়া করে।"
        },
        {
            "id": "chittagong_edu_005",
            "source_pure_bn": "শিক্ষকগণ শ্রেণিকক্ষে পাঠদান করছেন।",
            "reference_dialect": "মাস্টার সাব ক্লাসে পড়াইয়ে।"
        },
        {
            "id": "chittagong_edu_006",
            "source_pure_bn": "বিদ্যালয়ের খেলার মাঠ বিশাল ও সুন্দর।",
            "reference_dialect": "স্কুলর খেলার মাঠ বড় আর সুন্দর।"
        },
        {
            "id": "chittagong_edu_007",
            "source_pure_bn": "পরীক্ষার ফলাফল অত্যন্ত ভালো হয়েছে।",
            "reference_dialect": "পরীক্ষার ফলাফল খুব ভালো অইছে।"
        },
        {
            "id": "chittagong_edu_008",
            "source_pure_bn": "গ্রন্থাগারে অনেক মূল্যবান বই রয়েছে।",
            "reference_dialect": "লাইব্রেরিতে অনেক দামি বই আছে।"
        },
        {
            "id": "chittagong_edu_009",
            "source_pure_bn": "বিজ্ঞান প্রদর্শনীতে শিক্ষার্থীরা উদ্ভাবনী প্রকল্প উপস্থাপন করেছে।",
            "reference_dialect": "বিজ্ঞান প্রদর্শনীতে পোয়াইন নতুন জিনিস দেহাইছে।"
        },
        {
            "id": "chittagong_edu_010",
            "source_pure_bn": "প্রতিষ্ঠানটি দীর্ঘ ঐতিহ্য বহন করছে।",
            "reference_dialect": "এই প্রতিষ্ঠানর বড় ইতিহাস আছে।"
        },
        {
            "id": "chittagong_edu_011",
            "source_pure_bn": "শিক্ষকরা শিক্ষার্থীদের প্রতি অত্যন্ত যত্নশীল।",
            "reference_dialect": "মাস্টার সাবরা পোয়াইনের লগে খুব যত্ন করেন।"
        },
        {
            "id": "chittagong_edu_012",
            "source_pure_bn": "বার্ষিক সাংস্কৃতিক অনুষ্ঠান বেশ জাঁকজমকভাবে অনুষ্ঠিত হয়।",
            "reference_dialect": "বছরের সাংস্কৃতিক অনুষ্ঠান ভালোভাবে অয়।"
        },
        {
            "id": "chittagong_edu_013",
            "source_pure_bn": "ক্যান্টিনে শিক্ষার্থীরা বিরতির সময় একত্রিত হয়।",
            "reference_dialect": "ক্যান্টিনে বিরতির সময় পোয়াইন একসাথে হয়।"
        },
        {
            "id": "chittagong_edu_014",
            "source_pure_bn": "অভিভাবকরা সন্তানদের ভবিষ্যৎ নিয়ে স্বপ্ন দেখেন।",
            "reference_dialect": "বাবা-মায়েরা পোয়াইনের ভবিষ্যৎ নিয়ে স্বপ্ন দেখেন।"
        }
    ]
    ...
    print("\n" + "=" * 80)
    print("      BANGLARMUKH CHITTAGONG DIALECT TRANSLATION EVALUATION REPORT")
    print("=" * 80)
    ...
    out_path = os.path.join(os.path.dirname(__file__), "chittagong_dialect_results.json")
