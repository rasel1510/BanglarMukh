...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] (RangpurPipeline) %(message)s",
    handlers=[
        logging.StreamHandler(sys.stdout),
        logging.FileHandler(
            os.path.join(os.path.dirname(__file__), "rangpur_translation.log"),
            encoding="utf-8"
        )
    ]
)
logger = logging.getLogger("RangpurTranslator")
        ...
        try:
            ...
            class DummyProcessor:
                ...
                def batch_decode(self, sequences, **kwargs):
                    return ["বাংলাদেশের একটা নামকরা স্কুল, কোনঠে জ্ঞানের আলো ছড়িয়া পড়োছে বাহে।"]
            ...
            logger.info("BanglarMukh VLM engine ready for Rangpur evaluation.")
            return True
...
class TranslationCache:
    """Disk-backed JSON cache for storing and retrieving Rangpur translation outputs."""
    def __init__(self, filename: str = "rangpur_dialect_cache.json"):
...
# ------------------------------------------------------------------------------
# GEMINI 1.5 PRO RANGPUR TRANSLATOR
# ------------------------------------------------------------------------------
class GeminiRangpurTranslator:
    """
    Wraps Google Gemini 1.5 Pro to translate standard Bengali captions into
    the Rangpur regional dialect of Bangladesh.
    ...
    def __init__(self, api_key: Optional[str]):
        ...
        self.system_instruction = (
            "আপনি রংপুর অঞ্চলের উপভাষার (রংপুরী) একজন বিশেষজ্ঞ ভাষাবিদ। "
            "আপনার কাজ হলো প্রমিত বাংলা বাক্যকে রংপুরের স্থানীয় উপভাষায় রূপান্তর করা। "
            "রংপুরী উপভাষার প্রধান বৈশিষ্ট্যগুলি যেমন: সর্বনাম রূপান্তর ('আমি' -> 'মুই', 'আমরা' -> 'হামরা', "
            "'আমাদের' -> 'হামাগের', 'ছেলেমেয়ে' -> 'ছাওয়াপোয়া/ছাওয়া'), "
            "ক্রিয়া পদে '-চো' / '-চং' / '-ছে' প্রত্যয়, এবং সম্বোধন সূচক 'বাহে' বা 'নাগো' এর ব্যবহার নিশ্চিত করুন। "
            "শুধুমাত্র রূপান্তরিত রংপুরী বাক্যটি আউটপুট করুন, কোনো ব্যাখ্যা বা মন্তব্য ছাড়া।"
        )
    ...
    def translate_zero_shot(self, text: str) -> str:
        prompt = (
            "নিচের প্রমিত বাংলা বাক্যটিকে রংপুরী উপভাষায় রূপান্তর করুন:\n\n"
            f"বাক্য: {text}\n"
            "রূপান্তরিত বাক্য:"
        )
    ...
    def translate_few_shot(self, text: str) -> str:
        prompt = (
            "কাজ: প্রমিত বাংলাকে রংপুরী উপভাষায় রূপান্তর করুন।\n\n"
            "উদাহরণ ১:\n"
            "প্রমিত: বাংলাদেশের একটি বিখ্যাত শিক্ষাপ্রতিষ্ঠান, যেখানে জ্ঞানের আলো ছড়িয়ে পড়ছে।\n"
            "আউটপুট: বাংলাদেশের একটা নামকরা স্কুল, কোনঠে জ্ঞানের আলো ছড়িয়া পড়োছে বাহে।\n\n"
            "উদাহরণ ২:\n"
            "প্রমিত: শিক্ষার্থীরা মনোযোগ দিয়ে পাঠ গ্রহণ করছে।\n"
            "আউটপুট: ছাওয়াপোয়াকলা মনোযোগ দিয়া পড়ালেখা করোছে।\n\n"
    ...
    def translate_cot(self, text: str) -> str:
        prompt = (
            "আপনি একজন ভাষাবিদ। নিচের বাক্যটি ধাপে ধাপে রংপুরী উপভাষায় রূপান্তর করুন:\n"
            "ধাপ ১: বাক্যের সর্বনাম পরিবর্তন করুন (যেমন: আমরা->হামরা)।\n"
            "ধাপ ২: আঞ্চলিক শব্দাবলি বসান (যেমন: ছেলেমেয়ে->ছাওয়াপোয়া)।\n"
            "ধাপ ৩: ক্রিয়া পদে রংপুরী প্রত্যয় প্রয়োগ করুন (যেমন: করছে->করোছে)।\n"
            "ধাপ ৪: রংপুরী ভাষার চলন ও সম্বোধন সূচক 'বাহে' বসিয়ে বাক্যটি সাজান।\n"
            "সবশেষে 'চূড়ান্ত রংপুরী রূপ: ' লেবেল দিয়ে রূপান্তরিত বাক্যটি লিখুন।\n\n"
            f"প্রমিত বাক্য: {text}"
        )
        raw = self._call_with_retry(prompt)
        match = re.search(r"চূড়ান্ত রংপুরী রূপ:\s*(.+)", raw)
        if match:
    ...
    def _simulate(self, prompt: str) -> str:
        if "শিক্ষা প্রতিষ্ঠান" in prompt or "edu_001" in prompt:
            ...
            return "একটা পুরোনো ঐতিহ্যবাহী স্কুল, কোনঠে ছাওয়াপোয়াকলা পড়ালেখা করোছে।"
        elif "ঢাকা college" in prompt or "ঢাকা কলেজ" in prompt:
            return "ঢাকা কলেজ বাংলাদেশের সগাইচে পুরোনো কলেজ বাহে।"
        elif "মনোযোগ" in prompt:
            return "ছাওয়াপোয়াকলা মনোযোগ দিয়া পড়ালেখা করোছে।"
        return "রংপুরী উপভাষায় রূপান্তরিত বাক্য।"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="BanglarMukh Rangpur Dialect Pipeline.")
    ...
    parser.add_argument("--lora_path", type=str, default="./checkpoints/rangpur_lora.pt")
    ...
    logger.info("=" * 70)
    logger.info("  BanglarMukh Rangpur Translation Pipeline Starting")
    logger.info("=" * 70)
    ...
    translator = GeminiRangpurTranslator(api_key=args.api_key)

    samples = [
        {
            "id": "rangpur_edu_001",
            "source_pure_bn": "বাংলাদেশের একটি বিখ্যাত শিক্ষাপ্রতিষ্ঠান, যেখানে জ্ঞানের আলো ছড়িয়ে পড়ছে।",
            "reference_dialect": "বাংলাদেশের একটা নামকরা স্কুল, কোনঠে জ্ঞানের আলো ছড়িয়া পড়োছে বাহে।"
        },
        {
            "id": "rangpur_edu_002",
            "source_pure_bn": "এক ঐতিহ্যমণ্ডিত বিদ্যাপীঠ, যেখানে শিক্ষার্থীরা জ্ঞান অর্জন করে থাকেন।",
            "reference_dialect": "একটা পুরোনো ঐতিহ্যবাহী স্কুল, কোনঠে ছাওয়াপোয়াকলা পড়ালেখা করোছে।"
        },
        {
            "id": "rangpur_edu_003",
            "source_pure_bn": "ঢাকা college বাংলাদেশের অন্যতম প্রাচীন বিদ্যায়তন।",
            "reference_dialect": "ঢাকা college বাংলাদেশের সগাইচে পুরোনো কলেজ বাহে।"
        },
        {
            "id": "rangpur_edu_004",
            "source_pure_bn": "শিক্ষার্থীরা মনোযোগ দিয়ে পাঠ গ্রহণ করছে।",
            "reference_dialect": "ছাওয়াপোয়াকলা মনোযোগ দিয়া পড়ালেখা করোছে।"
        },
        {
            "id": "rangpur_edu_005",
            "source_pure_bn": "শিক্ষকগণ শ্রেণিকক্ষে পাঠদান করছেন।",
            "reference_dialect": "মাস্টাররা ক্লাসোত পড়ালেখা শিখাওছে।"
        },
        {
            "id": "rangpur_edu_006",
            "source_pure_bn": "বিদ্যালয়ের খেলার মাঠ বিশাল ও সুন্দর।",
            "reference_dialect": "স্কুলের খেলার মাঠখান মেলা বড় আর চকচকা সুন্দর।"
        },
        {
            "id": "rangpur_edu_007",
        ...
        },
        {
            ...
            "reference_dialect": "বইয়ের ঘরোত মেলা মূল্যবান বই আছে।"
        },
        {
            "id": "rangpur_edu_009",
            "source_pure_bn": "বিজ্ঞান প্রদর্শনীতে শিক্ষার্থীরা উদ্ভাবনী প্রকল্প উপস্থাপন করেছে।",
            "reference_dialect": "বিজ্ঞান মেলাত ছাওয়াপোয়াকলা নতুন নতুন জিনিস বানায় দেখাইছে।"
        },
        {
            "id": "rangpur_edu_010",
            "source_pure_bn": "প্রতিষ্ঠানটি দীর্ঘ ঐতিহ্য বহন করছে।",
            "reference_dialect": "এই স্কুলটা মেলা দিনের পুরোনো ঐতিহ্য নিয়া আছে।"
        },
        {
            "id": "rangpur_edu_011",
            "source_pure_bn": "শিক্ষকরা শিক্ষার্থীদের প্রতি অত্যন্ত যত্নশীল।",
            "reference_dialect": "মাস্টারগুলা ছাওয়াপোয়াদের খুব যত্ন করোছে।"
        },
        {
            "id": "rangpur_edu_012",
            "source_pure_bn": "বার্ষিক সাংস্কৃতিক অনুষ্ঠান বেশ জাঁকজমকভাবে অনুষ্ঠিত হয়।",
            "reference_dialect": "বছরের সাংস্কৃতিক অনুষ্ঠান খুব ধুমধাম করি হয় বাহে।"
        },
        {
            "id": "rangpur_edu_013",
            "source_pure_bn": "ক্যান্টিনে শিক্ষার্থীরা বিরতির সময় একত্রিত হয়।",
            "reference_dialect": "ছুটির সময় ছাওয়াকলা ক্যান্টিনোত একসাথে জড়া হয়।"
        },
        {
            "id": "rangpur_edu_014",
            "source_pure_bn": "অভিভাবকরা সন্তানদের ভবিষ্যৎ নিয়ে স্বপ্ন দেখেন।",
            "reference_dialect": "মা-বাপেরা ছাওয়াপোয়াদের ভবিষ্যৎ নিয়া মেলা আশা করোছে।"
        }
    ]
    ...
    print("\n" + "=" * 80)
    print("      BANGLARMUKH RANGPUR DIALECT TRANSLATION EVALUATION REPORT")
    print("=" * 80)
    ...
    out_path = os.path.join(os.path.dirname(__file__), "rangpur_dialect_results.json")
